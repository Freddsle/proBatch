    ...
    if (!.pb_missing_grouping_disabled(color_by)) {
        group_info <- .pb_prepare_grouped_missing_annotation(
            ...
            sample_message = paste0(
                "Stratified missingness-intensity plots require unique, ",
                "non-missing sample column names."
            )
        )
...
.pb_NA_intensity_cor_labels <- function(stats_df, has_group) {
    if ("pbf_name" %in% names(stats_df)) {
        assay_values <- as.character(stats_df$pbf_name)
        ...
        labels <- lapply(assays, function(assay_name) {
            ...
            assay_labels <- .pb_NA_intensity_cor_labels(
                assay_stats,
                has_group
            )
            if (!is.null(assay_labels) && nrow(assay_labels)) {
                assay_labels$pbf_name <- assay_name
            }
            assay_labels
        })
        labels <- Filter(
            function(label) !is.null(label) && nrow(label),
            labels
        )
        if (!length(labels)) {
            return(NULL)
        ...
        labels$pbf_name <- factor(labels$pbf_name, levels = assays)
        return(labels)
    }
    ...
    if (has_group) {
        ...
        rho <- cor(
            stats_df$mean_intensity,
            stats_df$prop_missing,
            method = "spearman",
            use = "complete.obs"
        )
        data.frame(
