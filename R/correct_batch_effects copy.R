...
.mask_imputed_measure <- function(df_long, measure_col, qual_col, qual_value, temp_suffix = "temp") {
    if (is.null(qual_col)) {
        return(df_long)
    }
    if (!(qual_col %in% names(df_long))) {
    ...
    df_long[[temp_measure_col]] <- ifelse(df_long[[qual_col]] == qual_value, NA, df_long[[measure_col]])
    attr(df_long, "temp_measure_col") <- temp_measure_col
    df_long
}
