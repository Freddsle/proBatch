...
.pb_plot_sample_corr_heatmap_single <- function(data_matrix,
    ...
    if (!is.null(samples_to_plot)) {
        ...
        mat_for_corr <- data_matrix[, samples_to_plot]
    } else {
        mat_for_corr <- data_matrix
    }
    # Use pairwise.complete.obs to avoid silently dropping the entire feature
    # set when any feature has missing values (which is the norm in proteomic
    # data); report how many features remained complete for context.
    if (anyNA(mat_for_corr)) {
        n_complete <- sum(stats::complete.cases(mat_for_corr))
        message(sprintf(
            "Sample correlation heatmap: %d/%d features fully observed; using pairwise.complete.obs.",
            n_complete, nrow(mat_for_corr)
        ))
    }
