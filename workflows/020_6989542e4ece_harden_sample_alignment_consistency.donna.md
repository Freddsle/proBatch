# Migrate Sample Alignment and Consistency Safeguards
...
Port independent core safeguards from mixed commit `6989542e4ece84f9fe210f3bf06b8254684e425e`, exclude PRONE and removed normalization behavior, and compare pinned split evidence under two maintainer review pauses.
...
1. Read manifest entry `post-sync-018-6989542e-harden-sample-alignment-consistency` in `{{ donna.lib.path("@/workflows/bec-ecoli-core-migration-manifest.md") }}`.
2. Inspect commit `6989542e4ece84f9fe210f3bf06b8254684e425e` against parent `d95dd736cb27d68fb2e21b20ab97d0f69826a663` using only pinned Git-object commands in `/home/yuliya/repos/other/proBatch`.
3. Non-generated paths are `R/correct_batch_effects.R`, `R/correlation-based_diagnostics.R`, `R/fit_non_linear.R`, `R/impute_PRONE.R`, `R/normalize.R`, and `R/utility_funcs.R`.
4. Core hunks: correction warns with dropped/added sample IDs and reorders survivors; `plot_sample_corr_heatmap` uses pairwise-complete correlation with context; `fit_nonlinear` protects callers from data.table mutation; group-aware normalization reports skipped groups; `check_sample_consistency` reports IDs unique to each side.
5. Exclude `imputePRONE_dm` alignment logic and PRONE dependencies. Also exclude skipped-group normalization because that API was deliberately removed from split core unless current retained behavior independently needs it.
6. Historical tests changed none. Focus on `test-correct_batch_effect.R`, `test-correlation_based_diagnostics.R`, `test-fit_non_linear.R`, and `test-utility_funcs.R`; use normalization tests only for a retained API.
7. Split comparator `29a7478dc7deea846a2c1ff1abd25a881e6f87db` already contains correction reordering, but lacks the pairwise heatmap hunk, caller-copy protection, and detailed consistency warning.
...
9. If behavior/ownership are clear, `{{ donna.lib.goto("port_bec_behavior") }}`; otherwise `{{ donna.lib.goto("blocked") }}`.
...
1. Detect the already-equivalent correction hunk before editing. Port only missing independent core safeguards and focused tests.
2. Test added/dropped sample diagnostics and ordering, incomplete correlation input, non-mutation of data.frame/data.table callers, and exact annotation/data mismatch reporting.
3. Keep PRONE and removed group-aware normalization out of core. Use `apply_patch`, preserve user changes, and never cherry-pick, stage, commit, amend, draft a commit message, regenerate documentation, or touch generated files.
4. Create `.session/donna/020-sample-alignment-consistency-notes.md` with evidence, Depmesh/dependency and skip decisions, focused checks, and exactly one of each:
   - `<!-- migration-id id=post-sync-018-6989542e-harden-sample-alignment-consistency -->`
...
set -euo pipefail
export LC_ALL=C
notes=".session/donna/020-sample-alignment-consistency-notes.md"
test -f "$notes"
test "$(grep -Fxc '<!-- migration-id id=post-sync-018-6989542e-harden-sample-alignment-consistency -->' "$notes")" -eq 1
...
Repair only retained core alignment/consistency behavior, tests, or notes after querying Depmesh. Preserve PRONE and removed-API exclusions. Then `{{ donna.lib.goto("verify_bec_port") }}` or `{{ donna.lib.goto("blocked") }}`.
...
1. Report each core hunk, already-equivalent and excluded behavior, tests, dependencies, and working-tree status.
...
2. Inspect exact targets in `R/correct_batch_effects.R`, `R/correlation-based_diagnostics.R`, `R/fit_non_linear.R`, `R/utility_funcs.R`, and their focused tests.
3. Confirm correction reordering is already present; reassess pairwise correlation, caller-copy protection, and detailed mismatch reporting. Record PRONE and removed normalization as no-target exclusions.
4. Never inspect/write an external working tree, import companion code, or access generated files.
5. Then `{{ donna.lib.goto("apply_split_adjustment") }}`, or `{{ donna.lib.goto("blocked") }}` if evidence conflicts.
...
1. Query Depmesh, then apply only independently justified retained-core hardening/tests. Record every target as adopted, equivalent, removed-API, or companion-excluded.
...
Repair only retained core hardening/tests or notes after querying Depmesh. Keep all exclusions. Then `{{ donna.lib.goto("verify_split_adjustment") }}` or `{{ donna.lib.goto("blocked") }}`.
...
1. Report split comparison, adopted/equivalent/excluded outcomes, tests, and working-tree status.
2. Do not stage, commit, or amend; the maintainer performs the commit. Inspect the exact in-scope unstaged diff and any in-scope untracked files without modifying the index. If the change set is nonempty, validate it against `@/specs/general/commits.md` and provide the complete copy-paste commit message in a fenced `text` block in the chat handoff. If one commit cannot accurately describe the change set, provide one validated message per coherent commit and identify each intended file set. If the change set is empty, state that no commit message is needed.
3. Do not complete this request until the maintainer explicitly resumes and supplies a 40-hex destination commit or an explicit no-new-commit decision.
...
Report retained/equivalent/excluded outcomes and review markers from `.session/donna/020-sample-alignment-consistency-notes.md`.
