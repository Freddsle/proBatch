...
handoff_pattern="provide the complete copy-paste commit"" message"
no_change_pattern="If the change set is empty, state that no commit message is"" needed."
...
4. Every foundation or commit child must implement this control flow with stable operation IDs:
   ...
   - pause at `review_bec_port` for manual developer analysis and commit, provide the required copy-paste commit-message handoff when the source-stage change set is nonempty, and forbid completing the action request until the developer explicitly resumes;
   ...
   - pause at `review_split_adjustment` for manual developer analysis and commit, provide the required copy-paste commit-message handoff when the split-stage change set is nonempty, and again require explicit resume;
...
for index in "${!workflow_artifacts[@]}"; do
    ...
    if test "$kind" = "foundation" || test "$kind" = "commit"; then
        ...
        test "$(grep -Fc "$handoff_pattern" "$workflow")" -eq 2
        test "$(grep -Fc "$no_change_pattern" "$workflow")" -eq 2
    else
        test "$(grep -Fxc 'id = "verify_residual_inventory"' "$workflow")" -eq 1
        test "$(grep -Fxc 'id = "review_residual_reports"' "$workflow")" -eq 1
        test "$(grep -Fc "$handoff_pattern" "$workflow")" -eq 1
        test "$(grep -Fc "$no_change_pattern" "$workflow")" -eq 1
    fi
done

test "$(grep -Fc "$handoff_pattern" workflows/001_bec-ecoli-core-migration.donna.md)" -eq 2
test "$(grep -Fc "$no_change_pattern" workflows/001_bec-ecoli-core-migration.donna.md)" -eq 2
...
2. Confirm every executable foundation/commit child has both maintainer review gates, both copy-paste commit-message handoffs for nonempty change sets, and both post-resume reverification gates; slot `017` remains executable; every reference-only slot has no workflow or catalog artifact; and the residual child has a deterministic 99-path inventory gate, repair loop, and report-review handoff.
...
5. Do not stage, create, or amend that commit. Inspect the exact in-scope unstaged diff and any in-scope untracked files without modifying the index. If the change set is nonempty, validate it against `{{ donna.lib.path("@/specs/general/commits.md") }}` and provide the complete copy-paste commit message in a fenced `text` block in the chat handoff. If one commit cannot accurately describe the change set, provide one validated message per coherent commit and identify each intended file set. If the change set is empty, state that no commit message is needed. Do not complete this action request until the developer explicitly asks to resume.
...
5. Do not stage, create, or amend that commit. Inspect the exact in-scope unstaged diff and any in-scope untracked files without modifying the index. If the change set is nonempty, validate it against `{{ donna.lib.path("@/specs/general/commits.md") }}` and provide the complete copy-paste commit message in a fenced `text` block in the chat handoff. If one commit cannot accurately describe the change set, provide one validated message per coherent commit and identify each intended file set. If the change set is empty, state that no commit message is needed. Do not complete this action request until the developer explicitly resumes.
