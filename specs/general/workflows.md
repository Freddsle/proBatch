...
- Purpose: analyze the core-owned API and the stopped split reference, define the exact BEC history scope, generate one numbered workflow per selected commit, and coordinate source-first and split-reference migration with maintainer-controlled review and commit pauses.
...
- Major stages: verify pinned reference inputs; resolve the non-linear history boundary; inventory API ownership, commit effects, and split implementation evidence; create and catalog numbered child workflows; run children in order; pause twice per commit for maintainer analysis and commits; audit residual stopped-split changes; create structured residual reports; run deterministic governance checks.
- Expected verification: pinned commits and the API map match recorded identities; every selected commit appears exactly once in the manifest and a child workflow; every generated workflow validates and renders; Depmesh returns workflow governance; focused source and test checks pass within each child; the maintainer explicitly resumes both review gates; final residual reports exist; generated-file-excluding `git diff --check` passes.
- Completion outcome: every selected commit has an explicit transferred, split-adjusted, skipped, or blocked outcome; all package changes remain committed only by the maintainer; and remaining stopped-split candidates and API decisions are recorded with rationale and ordering.
