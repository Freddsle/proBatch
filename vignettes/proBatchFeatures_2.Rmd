...
The package contains two methods for batch effects correction: ComBat from sva and removeBatchEffect from limma. Both of them can be accessed via the same function `pb_transform()`.

```{r}
# first extract sample annotations
sa <- colData(pbf) %>% as.data.frame()

# applying limma's removeBatchEffect
pbf <- pb_transform(
    pbf,
...
pbf
get_operation_log(pbf) %>% as_tibble()
```


The PCA shows that samples no longer cluster strictly by their center of origin, indicating that the batch effect has been removed
...
plot <- plot_PCA(
    ...
    shape_by = "Condition",
    fill_the_missing = NULL,
    base_size = 6, point_size = 3, point_alpha = 0.5
)
```

Every step is written to the operation log exactly once, so the full
pipeline can be recovered via `get_operation_log()` or
`pb_pipeline_name()`:

```{r}
get_operation_log(pbf_chain) %>% as.tibble()
```
...
## 3.7 Assessing the Processed Data
