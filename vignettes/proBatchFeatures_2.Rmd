...
FedProt paper [@burankova2025privacy]. The data contains
...
plot_sample_boxplots_on_assay(
    pbf,
    assay_name = "protein::raw",
    color_by = "Center",
    color_scheme = color_scheme
) +
    ggtitle("Raw Peptide Intensities")
```
Next, we'll perform a Principal Component Analysis (PCA) to get a high-level overview of the sample clustering. We expect to see a strong batch effect, where samples cluster by their center of origin rather than their biological condition.

```{r}
plot_pca_on_assay(pbf,
    assay_name = "peptide::raw",
    color_by = "Center",
    color_scheme = color_scheme,
    plot_title = "PCA on Raw Peptide Data"
)
```
...
ggplot(data.frame(nNA = na_counts$nNArow), aes(x = nNA)) +
    geom_histogram(bins = 30, fill = "steelblue") +
    geom_vline(xintercept = ncol(pbf[["peptide::raw"]]) * 0.5, color = "red", linetype = "dashed") +
    labs(
        title = "Distribution of Missing Values per Peptide",
        x = "Number of Samples with Missing Value",
...
Proteomics data is typically log-transformed to stabilize variance and make the data distribution more symmetric, which benefits many statistical methods. We'll apply a log₂ transformation to our filtered data.

```{r}
pbf <- pb_transform(
    object = pbf,
    from = "peptide::raw_filtered_on_raw",
    steps = "log2"
)
...
p1 <- proBatch::plot_sample_boxplots_on_assay(pbf,
    assay_name = "peptide::raw_filtered_on_raw",
    color_by = "Center",
    color_scheme = color_scheme
) +
    ggtitle("Before Log2 Transformation")
...
p2 <- proBatch::plot_sample_boxplots_on_assay(pbf,
    ...
    ggtitle("After Log2 Transformation")

# Arrange plots vertically
gridExtra::grid.arrange(p1, p2, ncol = 1)
```

The "After" plot shows that the distributions are now more symmetric and the variances across samples are more comparable. However, the differences in medians between samples are still apparent.

## 3.4 Median Normalization
...
## 3.5 Assessing the Processed Data
