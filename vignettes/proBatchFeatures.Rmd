...
knitr::opts_chunk$set(
    echo = TRUE, warning = TRUE, message = TRUE, fig.pos = "h",
    collapse = TRUE,
    comment = "#>",
...
Linear methods such as PCA are often complemented by non-linear
embeddings that emphasise local neighbourhood structure. The
`plot_TSNE()` and `plot_UMAP()` helpers follow the same interface as
`plot_PCA()`:

```{r plot_embeddings, fig.show='hold', fig.width=8, fig.height=4}
tsne_plot <- plot_TSNE(
    pbf,
    pbf_name = "protein::log2_on_raw",
    sample_id_col = "Run",
    ...
    perplexity = 15,
    random_seed = 42,
    plot_title = "t-SNE after log2 transform",
    point_size = 3,
    point_alpha = 0.5
)
...
umap_plot <- plot_UMAP(
    ...
    n_neighbors = 15,
    random_state = 42,
    plot_title = "UMAP after log2 transform",
    point_size = 3,
    point_alpha = 0.5
)
...
Both embeddings reproduce the lab-driven separation seen in the PCA
plots while emphasising finer structure within each group, offering a
complementary view on the same batch effects.
